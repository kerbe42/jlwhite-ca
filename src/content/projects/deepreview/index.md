...
**3. Infrastructure mapping.** Discovered hosts are resolved to hosting providers, geographies, and ASNs, building a map of where the subject's surface actually lives — often a mix of clouds, a forgotten legacy host, and a third-party SaaS or two.
...
**10. Capstone breach-risk profile.** Finally a capstone executive pass chains the weaknesses into concrete multi-step attack paths — not "you have an exposed admin panel" but how an exposed panel plus a leaked credential plus a stale TLS config compose into a plausible compromise — names the data that would be at risk, and gives a do-business verdict.

...
featured: false
cover: ./cover.png
coverAlt: A radial diagram of GYST — a JARVIS assistant at the centre surrounded by inventory, chores, food, notes, appointments, and PWA tiles
...
None of these are perfect on their own, which is the point — they get you to a draft you can correct in a few taps instead of a blank form you have to fill from scratch.

## JARVIS, the agentic assistant

The assistant — I call it JARVIS — is a Claude/OpenAI agentic tool-use loop with 40-plus tools. The design rule was full CRUD parity: anything you can do by clicking through the UI, the assistant has a tool for. The tools map directly onto the same mutations the UI calls, so there's no second-class command surface and no "the app can do this but the assistant can't." That parity is what makes it trustworthy to delegate to — it isn't a chatbot bolted onto the side, it's another way to drive the same app.
...
None of that is exotic, and that's the point — these are the table-stakes controls that home projects routinely skip, and skipping them is how a "harmless" home app becomes the soft target on your network.

## Closing

GYST runs in production on my own hardware, holding the real state of a household. The three commitments behind it are the same ones that run through the rest of my work: an LLM assistant should have real parity with the app it lives in, capture friction is the thing that actually kills self-hosted tools, and a home project still deserves real security controls. It was built solo with heavy AI pair-programming. Of all my projects it's the one with the lowest barrier to understanding — open the repo, stand it up on one box, and the way I think about putting an agent to work is right there in the tool surface.

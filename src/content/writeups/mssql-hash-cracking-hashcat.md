...
date: 2026-05-09
summary: 'A captured MSSQL login hash, a wordlist that runs dry, and the methodical climb from rockyou to rules to masks to a length-bounded brute force — plus the VMware gotcha that quietly drops you to CPU-only and triples your crack time.'
platform: TryHackMe
difficulty: Medium
...
3. **Use a hypervisor that *does* pass GPUs through.** This is the home-lab answer — Proxmox or ESXi support VMDirectPath / PCIe passthrough, so a VM can take full ownership of the card. Workstation is the wrong tool for GPU compute; a Type-1 hypervisor is the right one. (It's the kind of thing the [home lab](/lab/home-lab-overview) exists to let you try.)

...
date: 2026-06-12
summary: PMX-CLUSTER1 is a four-node Proxmox cluster on Lenovo M900 Tinys with deliberately local-only storage. Standing it up was easy; the bugs were the education — a maddening five-minute logout loop, version drift, a Trixie repo gauntlet, and stale NVMe references. The debugging is the interesting part.
room: Home lab
platform: Home lab
difficulty: Info
...
The compute core of the [home lab](/lab/home-lab-overview) is **PMX-CLUSTER1**: four Proxmox nodes, `PMX-HOST1` through `PMX-HOST4`, on `192.168.3.20`–`.23`, clustered over knet and quorate. The hardware is deliberately boring — four **Lenovo M900 Tiny** boxes, the kind that turn up cheap and sip power — which is the right call for a cluster whose job is to *run* infrastructure, not to be infrastructure.

Clustering four nodes is a fifteen-minute job. What took the time, and taught me the most, was everything that broke afterward. This is that list.
...
The logout loop stopped immediately. The lesson is the transferable part: **when the session, the proxy, and the clock are all clean, suspect the cluster's internal trust artifacts.** A distributed system has more "is this fresh and trusted?" checks than a single box, and a join event is exactly the kind of thing that scrambles them.
...
— which is just the release key for the new suite not being installed yet:
...
- **A join/rejoin event scrambles a cluster's internal state in non-obvious ways.** The five-minute logout wasn't a session bug or a clock bug — it was the cluster distrusting its own auth keys after a node came back. When the usual suspects are clean, suspect the trust material.
